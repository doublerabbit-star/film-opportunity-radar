      ...
      <main className="detail-main">
        <article className="detail-article">
          ...
          <header className="detail-hero">
            ...
            <div className="detail-score"><span>Opportunity score</span><strong>{opportunity.score.toFixed(1)}</strong></div>
          </header>

          <div className="detail-facts">
            <div><span>Source</span><strong>{opportunity.source}</strong></div>
            <div><span>Published</span><strong>{formatPublishedAt(opportunity.publishedAt)}</strong></div>
            <div><span>7-day trend</span><strong>{opportunity.trend}</strong></div>
            <div><span>Timing</span><strong>{opportunity.opportunityWindow}</strong></div>
          ...
          <div className="detail-body">
            <section className="detail-why">
              ...
              <aside className="timing-note"><Clock3 size={17} /><div><span>Publishing window</span><strong>{opportunity.opportunityWindow}</strong><p>Lead with analysis while the conversation is still forming.</p></div></aside>
            </section>
            <section className="detail-list-section">
              ...
              <ol>{opportunity.contentAngles.map((angle, index) => <li key={angle}><span>0{index + 1}</span><p>{angle}</p><ArrowUpRight size={18} /></li>)}</ol>
            </section>
